...
\begin{tikzpicture}[
    ...
    \draw[bonds]
    (Netz_in) edge[flow=$i_{\mathrm{in}}$, effort=$u_{\mathrm{in}}$] (ASM_Stator)
    ...
    % Mechanik
    \node (Welle) [bgelement, right=1.3cm of ASM_Rotor] {Welle};
    ...
    \draw[bonds] (SG_Stator) edge[flow=$i_{\mathrm{out}}$, effort=$u_{\mathrm{out}}$] (Last);
    
    % Regler
    \node (Summe) [sum, below=of Last] {};
    \node (Sollspannung) [input, right=of Summe, label=$U_{\mathrm{A,soll}}$] {};
    \node (Regler) [block, label={right:}] at ($(Summe)!1/3!(EM_Stator)$) {PID};
    \node (PWM) [pulse, label={right:PWM}] at ($(Summe)!2/3!(EM_Stator)$) {};
    \draw[signal] (Last) -- node[left]{$U_{\mathrm{A,ist}}$} (Summe);
